...
void main() {
    ...
    vec4 sum = vec4(0.0);
    sum += texture(texture0, fragTexCoord) * 0.294117;
    sum += texture(texture0, vec2(fragTexCoord.x - xStep, fragTexCoord.y)) * 0.235294;
    sum += texture(texture0, vec2(fragTexCoord.x + xStep, fragTexCoord.y)) * 0.235294;
    ...
    sum += texture(texture0, vec2(fragTexCoord.x - xStep * 3.0, fragTexCoord.y)) * 0.023529;
    sum += texture(texture0, vec2(fragTexCoord.x + xStep * 3.0, fragTexCoord.y)) * 0.023529;
